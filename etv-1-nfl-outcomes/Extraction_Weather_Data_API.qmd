...
\[use \_lst, \_df\] (need clarity)

NOTE: \[it seems like httr2 has A LOT of functionality that could reduce code and use of things like jsonlite, etc. worth investigating\]
...
\[Actual URL vs Query format, learn more about URL HTML format\] (need clarity on this)

{Question: What is the 'query' format vs the URL glue form? }
...
First, we assign the URL we created to an httr2:: request object, called req. Then, we use httr2::req_perform() to call the OpenWeather API and assign the response to a variable. We then use httr2::resp_content_type() to see what our result looks like.
...
{Question: What does application mean? What is JSON?}
...
{Question: What are other status codes we know? What about 404?}
...
{Question: What else could be added here? Subtracted? Why the output string?}
...
{Question: Why wrap as.character() around the date range?}
...
{Any thoughts at this point? What benefits do we get from inspecting the data structure?}
